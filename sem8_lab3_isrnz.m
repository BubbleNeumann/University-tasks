...
I_a = @(x, y, z) (sin(k*a.*X/(2*z))./(k*a.*X/(2*z))).^2 ...
    .*(sin(k*b.*Y/(2*z))./(k*b.*Y/(2*z))).^2; % аналитическое решение для интенсивности дифракции на экране для заданных координат xx и yy
    
%расчет интегралов Рэлея-Зоммерфельда первого рода для каждой точки на экране для заданных расстояний z1 и z2
for i = 1:(2*screen_size+1)
  for j = 1:(2*screen_size+1)
    ...
    % считаем интегралы Рэлея-Зоммерфельда первого рода на прямоугольной апертуре (по всей площади) с размерами 5λ×3λ
    % используется для описания дифракции света на препятствиях.
    % учитывает вклад каждой точки на поверхности апертуры в формирование поля на удаленном экране
...
% делим на максимальное значение
I_a1 = I_a1 / max(I_a1(:));
